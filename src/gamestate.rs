...
pub async fn auctions_per_day(arcdb: &Arc<tokio_postgres::Client>, day: i16) -> i32 {
    let row = arcdb
        .query_one(
            "SELECT COUNT(DISTINCT nr) from item where day = $1 ;",
            &[&day],
        )
        .await
        .expect("database failure counting auction days");
...
impl GameState {
    pub async fn fromdb(arcdb: &Arc<tokio_postgres::Client>) -> GameState {
        let maybegamestaterow = &arcdb
            .query_opt("SELECT phase,deadline,rate FROM gamestate;", &[])
            .await
            .expect("database failure fetching gamestate");
    ...
    pub async fn advance(self: GameState, arcdb: &Arc<tokio_postgres::Client>) -> GameState {
        match self {
            GameState::Auction {
                ...
                match auctions_on_newday {
                    0 => {
                        ...
                        let _rows = &arcdb
                            .query("INSERT INTO gamestate (phase) VALUES ($1);", &[&-1i16])
                            .await
                            .expect("database failure");
                        GameState::Finished
                    }
            ...
            _ => {
                panic!("advance called when auction was not open")
            }
        }
    }
}
