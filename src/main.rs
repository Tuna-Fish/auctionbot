...
use serenity::{
    ...
    model::{channel::Message, gateway::Ready, id::ChannelId, id::UserId},
    prelude::*,
};
use std::sync::Arc;
    ...
    if let Ok(channel) = user.create_dm_channel(&ctx).await {
        ...
        s.push_str(&match newstate {
            ...
            GameState::Auction {
                ...
                format!(
                    "auctions for day {} are open. {}",
                    day,
                    pretty_print_deadline(deadline)
                )
            }
            _ => "".to_string(),
        });
...
async fn spam(ctx: Context, newstate: GameState, oldstate: GameState) {
    ...
    let (oldday, _olddeadline, _oldrate) = if let GameState::Auction {
        day,
        deadline,
        rate,
    } = oldstate
    {
        ...
        panic!("auction during non-auction day?")
    };

    match arcdb
        .query_opt("SELECT id FROM channel", &[])
        .await
        .expect("dberror")
    {
        Some(row) => {
    ...
    for row in arcdb
        .query("SELECT id,points FROM discorduser", &[])
        .await
        .expect("dberror")
    {
...
async fn tick(ctx: Context) {
    ...
    if shouldauction {
        let (newgamestate, oldgamestate) = auction(&ctx, true).await.expect("auction failed");
...
#[group]
#[commands(users, help, wins, register)]
struct General;
