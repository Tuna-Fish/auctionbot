use crate::gamestate::auctions_per_day;
...
pub async fn auction(ctx: &Context, advance: bool) -> Option<(GameState, GameState)> {
    ...
    let day = match *gamestatewriteguard {
        GameState::Auction {
            day,
            deadline: _,
            rate: _,
        ...
        _ => {
            error!("called auction on non-auction day");
            return None;
        }
    };
    ...
    let userrows = arcdb
        .query("SELECT id, name, points FROM discorduser", &[])
        .await
        .expect("muh users");
    ...
    for row in userrows {
        ...
        info!(
            "prior to auction on day {}, user {}/{} had {} points",
            day, user.name, user.id, user.points
        );
    ...
    for mut bids in outer {
        ...
        loop {
            ...
            let winner = bids
                .pop()
                .expect("no highest bid in a list with >0 entries");
            ...
            info!(
                " user: {} won bid for {} with {} points",
                users.get(&winner.user).unwrap().name,
                winner.item,
                cost
            );
            arcdb
                .query(
                    "INSERT INTO win (userid,item,day,cost) VALUES($1,$2,$3,$4)",
                    &[&winner.user, &winner.item, &day, &cost],
                )
                .await
                .expect("failed to store winning bid");
            arcdb
                .query(
                    "UPDATE discorduser SET points = $1 WHERE id = $2;",
                    &[
                        &users.get(&winner.user).unwrap().points,
                        &users.get(&winner.user).unwrap().id,
                    ],
                )
                .await
                .expect("failed to store winner's points");
